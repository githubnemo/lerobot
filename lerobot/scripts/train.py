...
def train(cfg: TrainPipelineConfig):
    ...
    for _ in pbar:
        ...
        if is_val_step and val_indices is not None:
            ...
            logging.info(f"Validation - Step {step}: loss={val_loss:.4f}, samples={val_n_samples}, time={val_time:.2f}s")
...
def create_train_val_splits(cfg: TrainPipelineConfig, dataset: LeRobotDataset) -> tuple[list[int], list[int] | None]:
    """Create train/validation index splits from the full dataset.
    
    This split is deterministic and reproducible:
    - Always splits at episode boundaries (maintains episode integrity)
    - Uses the same episodes for train/val across runs with the same val_split
    - Does not depend on random seeding for the split itself
    """
    ...
    # Split episodes deterministically (always same episodes for same val_split)
    num_train_episodes = math.floor(total_episodes * (1 - cfg.val_split))
    ...
    val_indices = []
    for ep_idx in range(num_train_episodes, total_episodes):
        start_idx = episode_data_index["from"][ep_idx].item()
        end_idx = episode_data_index["to"][ep_idx].item()
        val_indices.extend(range(start_idx, end_idx))
    
    logging.info(f"Dataset split - Total episodes: {total_episodes}")
    logging.info(f"Training episodes: {num_train_episodes} ({100*(1-cfg.val_split):.1f}%)")
    logging.info(f"Validation episodes: {total_episodes - num_train_episodes} ({100*cfg.val_split:.1f}%)")
    logging.info(f"Training frames: {len(train_indices)}")
    logging.info(f"Validation frames: {len(val_indices)}")
    logging.info(f"Validation split is deterministic - always uses episodes {num_train_episodes}-{total_episodes-1}")
