#!/usr/bin/env python3
"""Build resumable train/validation LTX-2.5 feature caches."""
...
DEFAULT_TRAIN_OUTPUT = Path("/home/anton/.cache/video-vam/ltx25-train0-31-stride3-pool2")
DEFAULT_VAL_OUTPUT = Path("/home/anton/.cache/video-vam/ltx25-val32-39-stride20-pool2")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--val-output-dir", type=Path, default=DEFAULT_VAL_OUTPUT)
...
def disk_guard(args: argparse.Namespace, selected: list[tuple[str, int, int]]) -> dict[str, Any]:
    ...
    if safe_requirement > free:
        raise RuntimeError(
            f"insufficient disk for provenance-tagged {args.context_transform} cache: "
    ...
    decision = {
        ...
        "storage_strategy": (
            "deterministic per-frame adaptive average pool 15x20 -> 8x10, stored BF16 per entry"
            if args.context_transform == "pool2"
            else "unpooled 8x15x20 latent grid flattened T,H,W,C and stored BF16 per entry"
        ),
        "representation_change": (
            "explicit pool2 spatial reduction; temporal grid and 4096 channels preserved"
            if args.context_transform == "pool2"
            else "none; exact raw hidden tokens stored in T,H,W,C order"
        ),
    }
...
def manifest_payload(
    *,
    ...
    return {
        ...
        "provenance": {
            "builder": "scripts/video_vam/build_ltx_feature_cache.py",
...
def existing_entry(path: Path, *, split: str, episode: int, frame: int, noise_seed: int) -> dict[str, Any]:
    item = verify_ltx_feature_cache(path)
...
def build(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    try:
        for index, (split, episode, frame) in enumerate(selected, 1):
            ...
            if args.resume and path.exists():
                entry = existing_entry(path, split=split, episode=episode, frame=frame, noise_seed=noise_seed)
                print(f"resume verified {index}/{len(selected)} {split} {episode}/{frame}", flush=True)
            else:
                ...
                context = (
                    (
                        pool2_ltx_context(extraction.hidden_grid)
                        if args.context_transform == "pool2"
                        else extraction.tokens
                    )
        ...
        extractor.close()
    if producer is None:
        first = verify_ltx_feature_cache(train_output / entries_by_split["train"][0]["safetensors"])
    ...
    for split, output_dir, stride in (
        ...
        write_ltx_cache_manifest(payload, manifest, overwrite=bool(args.resume or args.overwrite))
        loaded = load_ltx_cache_manifest(manifest)
