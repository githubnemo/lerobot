#!/usr/bin/env bash
# Smoke-first Cosmos state_t=2 video-LoRA SmolExpert with layer attention.
set -Eeuo pipefail
...
RUN_ROOT="$CACHE/runs/cosmos2b-videolora-statet2-attn-smolexpert-20260829"
TRAIN_CACHE="$CACHE/cosmos-videolora-train0-31-stride3-statet2-unpooled-layers"
VAL_CACHE="$CACHE/cosmos-videolora-val32-39-stride20-statet2-unpooled-layers"
if (( SMOKE_ONLY == 1 )); then
    TRAIN_CACHE="$CACHE/cosmos-videolora-smoke-ep0-statet2-unpooled-layers"
    VAL_CACHE="$CACHE/cosmos-videolora-smoke-val32-39-statet2-unpooled-layers"
fi
...
if [[ ! -f "$BEST_LORA" || ! -f "$CHECKPOINT" || ! -f "$TOKENIZER" || ! -f "$SPLIT" ]]; then
    printf 'missing LoRA, Cosmos checkpoint, tokenizer, or split file\n' >&2
    exit 1
fi

stage=gpu-lock
acquire_gpu_lock cosmos-statet2-lora-attn
...
"$PYTHON" -m scripts.video_vam.train_smolexpert_on_cosmos \
    ...
    --run-name cosmos2b-videolora-statet2-attn-smolexpert-20260829 \
    --overwrite
