#!/usr/bin/env python3
"""Train SmolExpert downstream action policy on extracted Cosmos 7B intermediate features.

Injects Cosmos-1.0-Diffusion-7B intermediate representations (layers 14 & 20)
...
class Cosmos7BFeatureDataset(Dataset):
    """Dataset loading cached Cosmos 7B feature artifacts."""
    ...
    def __init__(self, cache_dir: Path | str) -> None:
        ...
        self.feature_dim = self.manifest.get("entries", [{}])[0].get("feature_dim", 4096)
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        features = data["features"]  # [1, S, C] or [S, C]
        if features.ndim == 3:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train SmolExpert downstream action policy on Cosmos 7B features."
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=None,
        help="Path to directory containing cached Cosmos 7B features and manifest.json.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/home/anton/.cache/video-vam/runs/smolexpert-cosmos7b"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Input feature dimension (e.g. 4096 for single layer, 8192 for concat 14+20). Auto-detected if manifest is present.",
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
...
def main() -> int:
    ...
    print(f"[SmolExpert Cosmos 7B] Target device: {device}")
    args.output_dir.mkdir(parents=True, exist_ok=True)

    if args.dry_run:
        print("[SmolExpert Cosmos 7B] --- RUNNING CPU SYNTHETIC DRY-RUN ---")
        input_channels = args.input_channels or 128  # Matches test dry-run cache dimension
        ...
        print(f"[SmolExpert Cosmos 7B] Starting {dry_run_steps} dry-run optimization steps...")
        decoder.train()
        for step in range(dry_run_steps):
            ...
            if (step + 1) % max(1, dry_run_steps // 5) == 0 or step == dry_run_steps - 1:
                print(f"[SmolExpert Cosmos 7B] Step {step + 1}/{dry_run_steps} - Loss: {loss.item():.4f}")

        # Test action prediction / inference
        decoder.eval()
        with torch.no_grad():
            ...
            print(f"[SmolExpert Cosmos 7B] Action prediction verified: shape {pred_action.shape}")

            metrics = compute_evaluation_metrics(pred_action, action_tensor)
            print(
                f"[SmolExpert Cosmos 7B Evaluation Metrics] RMSE: {metrics['rmse']:.4f} | "
                f"H1: {metrics['h1']:.4f} | First-5: {metrics['first5']:.4f}"
            )
            metrics_path = args.output_dir / "eval_metrics.json"
            with open(metrics_path, "w", encoding="utf-8") as f:
                json.dump(metrics, f, indent=2)
            print(f"[SmolExpert Cosmos 7B] Evaluation metrics saved to {metrics_path}")

        # Save checkpoint
        ckpt_path = args.output_dir / "smolexpert_cosmos7b_dryrun.safetensors"
        save_file({f"model.{k}": v.cpu() for k, v in decoder.state_dict().items()}, str(ckpt_path))
        print(f"[SmolExpert Cosmos 7B] Checkpoint saved successfully to {ckpt_path}")
        return 0
    ...
    dataset = Cosmos7BFeatureDataset(args.cache_dir)
    input_channels = args.input_channels or dataset.feature_dim
    print(f"[SmolExpert Cosmos 7B] Loaded dataset with {len(dataset)} samples, feature_dim: {input_channels}")
    ...
    print(f"[SmolExpert Cosmos 7B] Train samples: {train_size}, Validation samples: {val_size}")
    ...
    print(
        f"[SmolExpert Cosmos 7B] Training until convergence (max_steps={args.max_steps}, eval_interval={eval_interval}, patience={patience})...",
        flush=True,
    )
    decoder.train()
    while step < args.max_steps:
        for batch in train_loader:
            ...
            if step % 50 == 0 and step % eval_interval != 0:
                print(
                    f"[SmolExpert Cosmos 7B] Step {step}/{args.max_steps} | Train Loss: {loss.item():.4f}",
                    flush=True,
                )
            ...
            if step % eval_interval == 0:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    patience_counter += 1
                print(
                    f"[SmolExpert Cosmos 7B] Step {step}/{args.max_steps} | "
                ...
                if patience_counter >= patience:
                    print(
                        f"[SmolExpert Cosmos 7B] Convergence reached! Early stopping triggered at step {step} (best val loss: {best_val_loss:.4f})",
                        flush=True,
                    )
                    break
                decoder.train()

            if step >= args.max_steps or patience_counter >= patience:
                break
        if patience_counter >= patience:
            break
    ...
    if best_weights_path.is_file():
        ...
        print(f"[SmolExpert Cosmos 7B] Loaded best checkpoint (val loss: {best_val_loss:.4f})")

    ckpt_path = args.output_dir / "smolexpert_cosmos7b_final.safetensors"
    save_file({f"model.{k}": v.cpu() for k, v in decoder.state_dict().items()}, str(ckpt_path))
    print(f"[SmolExpert Cosmos 7B] Training complete. Saved to {ckpt_path}")
