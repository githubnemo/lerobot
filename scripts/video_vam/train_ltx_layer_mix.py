#!/usr/bin/env python3
"""Train World2Action on a learned scalar mix of aligned LTX depths."""
...
LTXLayerMixer = LTXLayerScalarMix | LTXLayerAttentionMix | LTXLayerGatedMix
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=LTX_LAYER_PROBE_DEPTHS,
        help="Comma-separated subset of cached layers; use one/two layers for confirmation runs.",
    )
    parser.add_argument("--mixer", choices=("scalar", "attention", "gated"), default="scalar")
    parser.add_argument("--attn-width", type=int, default=2048)
    parser.add_argument("--attn-heads", type=int, default=8)
    parser.add_argument("--gate-per-channel", action="store_true")
    ...
    if args.train_manifest is None:
        args.train_manifest = DEFAULT_CACHE_ROOT / (
            f"ltx25-layer-probe-train0-31-stride3-{args.context_transform}/manifest.json"
        )
    if args.val_manifest is None:
        args.val_manifest = DEFAULT_CACHE_ROOT / (
            f"ltx25-layer-probe-val32-39-stride20-{args.context_transform}/manifest.json"
        )
    if args.output_dir is None:
        args.output_dir = DEFAULT_CACHE_ROOT / (f"runs/ltx25-layer-mix-{args.context_transform}-w2a-20260826")
    if args.wandb_run_name is None:
        args.wandb_run_name = f"ltx25-layer-mix-{args.context_transform}-w2a-s{args.seed}-20260826"
    return args
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if (
        not args.layers
        or tuple(sorted(set(args.layers))) != args.layers
        or not set(args.layers).issubset(LTX_LAYER_PROBE_DEPTHS)
    ):
        raise ValueError(f"--layers must be a sorted unique subset of {LTX_LAYER_PROBE_DEPTHS}")
...
def batch_tensors(
    dataset: LTXLayerMixCacheDataset,
    ...
    return (
        ...
        {
            layer: torch.cat([item.contexts[layer] for item in items]).to(device=device, dtype=torch.bfloat16)
            for layer in layers
        },
...
def evaluate(
    ...
    *,
    ...
    prefix_squared = {
        1: torch.zeros((), dtype=torch.float64, device=device),
        5: torch.zeros((), dtype=torch.float64, device=device),
    }
    prefix_count = {
        1: torch.zeros((), dtype=torch.float64, device=device),
        5: torch.zeros((), dtype=torch.float64, device=device),
    ...
    for start in range(0, len(entries), batch_size):
        ...
        state, action, raw_contexts, padding = batch_tensors(dataset, selected, device, layers)
        ...
        with autocast_context(device):
            context = adapter(mixer(raw_contexts))
        ...
        for index, probe in enumerate(probes):
            ...
            for horizon in (1, 5):
                ...
                prefix_squared[horizon] += (squared[:, :horizon] * prefix_valid).sum().to(torch.float64)
                prefix_count[horizon] += prefix_valid.sum().to(torch.float64)
    ...
    if (
        ...
        or bool((physical_count == 0).any().item())
        or any(count.item() == 0 for count in prefix_count.values())
    ):
    ...
    return {
        ...
        "val_action_rmse_h1_degrees": math.sqrt(float(prefix_squared[1].item() / prefix_count[1].item())),
        "val_action_rmse_first5_degrees": math.sqrt(float(prefix_squared[5].item() / prefix_count[5].item())),
        ...
        "val_metric_contract": (
            "global masked mixed-unit RMSE over SO-101 joint targets; "
            "five arm joints are degrees and gripper is range_0_100"
        ),
...
def model_tensors(
    decoder: World2ActionDecoder, adapter: LTXContextAdapter, mixer: LTXLayerMixer
...
def load_model(
    path: Path, decoder: World2ActionDecoder, adapter: LTXContextAdapter, mixer: LTXLayerMixer
) -> None:
    ...
    if len(decoder_state) + len(adapter_state) + len(mixer_state) != len(tensors):
...
def save_weights(
    ...
    save_file(model_tensors(decoder, adapter, mixer), str(temporary))
    os.replace(temporary, path)
...
def train(args: argparse.Namespace) -> Path:
    ...
    train_manifest = load_layer_mix_manifest(train_manifest_path)
    val_manifest = load_layer_mix_manifest(val_manifest_path)
    ...
    train_transform = train_manifest.payload["provenance"]["context_transform"]
    val_transform = val_manifest.payload["provenance"]["context_transform"]
    if train_transform != args.context_transform or val_transform != args.context_transform:
        raise ValueError(
            "requested context transform does not match both cache manifests: "
            f"requested={args.context_transform}, train={train_transform}, val={val_transform}"
        )
    ...
    train_dataset = LTXLayerMixCacheDataset(train_manifest)
    val_dataset = LTXLayerMixCacheDataset(val_manifest)
    ...
    if args.mixer == "scalar":
        mixer: LTXLayerMixer = LTXLayerScalarMix(args.layers, seed=args.seed)
        ...
        mixer = LTXLayerGatedMix(
            args.layers,
            per_channel=args.gate_per_channel,
            seed=args.seed,
        )
    mixer = mixer.to(device=device, dtype=torch.float32)
    mixer_parameter_count = sum(parameter.numel() for parameter in mixer.parameters())
    print(
        f"MIXER_VARIANT={args.mixer} MIXER_TRAINABLE_PARAMETERS={mixer_parameter_count}",
        flush=True,
    )
    ...
    parameters.extend(parameter for parameter in adapter.parameters() if parameter.requires_grad)
    ...
    best_h1_degrees = math.inf
    best_first5_degrees = math.inf
    best_per_joint_degrees: list[float] | None = None
    best_mix_diagnostics: dict[str, Any] | None = None
    ...
    if args.resume:
        ...
        load_model(last_weights, decoder, adapter, mixer)
        ...
        best_h1_degrees = float(metadata["best_val_action_rmse_h1_degrees"])
        best_first5_degrees = float(metadata["best_val_action_rmse_first5_degrees"])
        best_per_joint_degrees = [
            float(value) for value in metadata["best_val_action_rmse_per_joint_degrees"]
        ]
        best_mix_diagnostics = metadata.get("best_mix_diagnostics")
    ...
    provenance = {
        "experiment": (f"frozen_ltx25_multidepth_{args.context_transform}_{args.mixer}_mix_world2action"),
        "dataset": {
        ...
        "layer_mix": {
            ...
            "architecture": {
                "scalar": (
                    "separate non-affine LayerNorm per layer; learned scalar softmax; global learned gain"
                ),
                "attention": (
                    "separate non-affine LayerNorm per layer; broadcast learned query; "
                    "layer-axis multi-head SDPA; projected residual around uniform mean; "
                    "global learned gain"
                ),
                "gated": (
                    "separate non-affine LayerNorm per layer; mean-content token gate; "
                    "optional additive per-layer/channel log-weights; global learned gain"
                ),
            }[args.mixer],
            "logit_seed": args.seed,
            "logit_init_std": 1.0e-3 if args.mixer == "scalar" else None,
            "attn_width": args.attn_width if args.mixer == "attention" else None,
            "attn_heads": args.attn_heads if args.mixer == "attention" else None,
            "gate_per_channel": args.gate_per_channel if args.mixer == "gated" else None,
            "context_transform": args.context_transform,
            "context_grid": list(ltx_context_grid(args.context_transform)),
            "token_count_after_mix": ltx_layer_probe_tokens(args.context_transform),
            "attention_over_layers": args.mixer == "attention",
            "trainable_parameters": mixer_parameter_count,
        },
        "adapter": {
            "input_channels": 4096,
        ...
        "training": {
            **config_payload(args),
            ...
            "validation_aggregation": (
                "global masked mixed-unit RMSE; five arm joints in degrees, gripper range_0_100"
            ),
            "executed_prefix_metrics": ["h=1", "mean first 5"],
        },
        "runtime": {
    ...
    try:
        while step < args.max_steps:
            ...
            for accumulation in range(args.grad_accum_steps):
                ...
                state, action, raw_contexts, padding = batch_tensors(
                    train_dataset, entries, device, args.layers
                )
                ...
                while remaining > 0:
                    ...
                    with autocast_context(device):
                        repeated_contexts = {layer: repeat(raw_contexts[layer]) for layer in args.layers}
                        context = adapter(mixer(repeated_contexts))
                    ...
                    del context, loss, scaled, repeated_contexts
                del state, action, raw_contexts, padding
            ...
            record: dict[str, Any] = {
                "step": step,
                "mixer": args.mixer,
                "mixer_trainable_parameters": mixer_parameter_count,
                ...
                "val_action_rmse_h1_degrees": None,
                "val_action_rmse_first5_degrees": None,
            }
            ...
            if should_validate:
                ...
                _, _, diagnostic_contexts, _ = batch_tensors(
                    val_dataset, (val_manifest.entries[0],), device, args.layers
                )
                mix_diagnostics = mixer.diagnostics(diagnostic_contexts)
                for layer, weight in mix_diagnostics["weights"].items():
                    record[f"layer_weight_{layer}"] = weight
                for layer, norm in mix_diagnostics["mean_contribution_norms"].items():
                    record[f"layer_contribution_norm_{layer}"] = norm
                record["layer_mix_gain"] = mix_diagnostics["gain"]
                record["layer_weight_dispersion"] = mix_diagnostics.get("weight_dispersion", 0.0)
                ...
                if improved:
                    ...
                    best_h1_degrees = float(validation["val_action_rmse_h1_degrees"])
                    best_first5_degrees = float(validation["val_action_rmse_first5_degrees"])
                    best_per_joint_degrees = [
                        float(value) for value in validation["val_action_rmse_per_joint_degrees"]
                    ]
                    best_mix_diagnostics = copy.deepcopy(mix_diagnostics)
                ...
                print(
                    ...
                    f"val_h1_rmse_deg={validation['val_action_rmse_h1_degrees']:.6g} "
                    f"val_first5_rmse_deg={validation['val_action_rmse_first5_degrees']:.6g} "
            ...
            if should_validate:
                metadata = {
                    **provenance,
                    ...
                    "best_val_action_rmse_h1_degrees": best_h1_degrees,
                    "best_val_action_rmse_first5_degrees": best_first5_degrees,
                    "best_val_action_rmse_per_joint_degrees": best_per_joint_degrees,
                    "best_mix_diagnostics": best_mix_diagnostics,
                ...
                save_weights(last_weights, decoder, adapter, mixer)
                ...
                if improved:
                    ...
                    save_weights(output / "best.safetensors", decoder, adapter, mixer)
    ...
    result = {
        ...
        "metric_contract": {
            ...
            "val_action_rmse_degrees": (
                "legacy metric name: global masked mixed-unit RMSE over five degree joints "
                "and range_0_100 gripper"
            ),
            "val_action_rmse_normalized": "global masked RMSE after train-split min/max normalization",
            "val_action_rmse_h1_degrees": "global masked mixed-unit RMSE at executed horizon 1",
            "val_action_rmse_first5_degrees": (
                "global masked mixed-unit RMSE over the first five executed actions"
            ),
