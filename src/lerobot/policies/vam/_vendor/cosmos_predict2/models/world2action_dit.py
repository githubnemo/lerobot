...
from .._compat import create_cuda_graph, log
from ..networks.selective_activation_checkpoint import CheckpointMode, SACConfig
...
def _load_transformer_engine():
    try:
        ...
        raise RuntimeError(
            "Transformer Engine is required during official World2Action model construction for TE RMSNorm; the selected torch attention backend does not remove this dependency."
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            "Transformer Engine API mismatch: its DotProductAttention API is unavailable; "
            "check the installed Transformer Engine version."
        ) from exc
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Transformer Engine API mismatch: apply_rotary_pos_emb was found neither at "
                "transformer_engine.pytorch.attention.rope (current API) nor at "
                "transformer_engine.pytorch.attention (legacy API); check the installed "
                "Transformer Engine version."
            ) from exc
    return te_module, DotProductAttention, apply_rotary_pos_emb
...
def torch_attention_op(
    q_B_S_H_D: torch.Tensor, k_B_S_H_D: torch.Tensor, v_B_S_H_D: torch.Tensor
) -> torch.Tensor:
    ...
    Returns:
        ...
        matching the common backend contract used by `Attention.compute_attention`.
    """
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        te_module, _, rotary_pos_emb = _load_transformer_engine()
        self.q_norm = te_module.pytorch.RMSNorm(self.head_dim, eps=1e-6)
        self._rotary_pos_emb = rotary_pos_emb

        self.k_proj = nn.Linear(context_dim, inner_dim, bias=False)
        self.k_norm = te_module.pytorch.RMSNorm(self.head_dim, eps=1e-6)
        ...
        if self.backend == "transformer_engine":
            _, dot_product_attention, _ = _load_transformer_engine()
            self.attn_op = dot_product_attention(
                self.n_heads,
                self.head_dim,
    ...
    def compute_qkv(
        ...
        def apply_norm_and_rotary_pos_emb(
            ...
            if self.is_selfattn and rope_emb is not None:  # only apply to self-attention!
                q = self._rotary_pos_emb(q, rope_emb, tensor_format=self.qkv_format, fused=True)
                k = self._rotary_pos_emb(k, rope_emb, tensor_format=self.qkv_format, fused=True)
            return q, k, v
...
class ContinuousSinusoidalPosEmb(nn.Module):
    ...
    def forward(self, timesteps_B_T: torch.Tensor) -> torch.Tensor:
        fraction = torch.linspace(
            0.0, 1.0, self._num_channels // 2, device=timesteps_B_T.device, dtype=torch.float64
        )
        ...
        return torch.cat(
            (torch.sin(sinusoid_input_B_T_D), torch.cos(sinusoid_input_B_T_D)), dim=-1
        ).bfloat16()
...
class PairTimestepEmbedding(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, rank: int, use_adaln_lora: bool = True):
        ...
        self.C = nn.Linear(
            out_features, rank, bias=True
        )  # subtract bias-bias term later to avoid global bias
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.use_adaln_lora:
            ...
            shift_cross_attn_B_T_D, scale_cross_attn_B_T_D, gate_cross_attn_B_T_D = (
                self.adaln_modulation_cross_attn(emb_B_T_DorR).chunk(3, dim=-1)
            )
            shift_self_attn_B_T_D, scale_self_attn_B_T_D, gate_self_attn_B_T_D = (
                self.adaln_modulation_self_attn(emb_B_T_DorR).chunk(3, dim=-1)
            )
            shift_mlp_B_T_D, scale_mlp_B_T_D, gate_mlp_B_T_D = self.adaln_modulation_mlp(emb_B_T_DorR).chunk(
                3, dim=-1
            )
...
class World2ActionDIT(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embedding = nn.Parameter(
            torch.empty((1, self.max_horizon, self.model_channels), dtype=torch.float32)
        )

        self.use_adaln_lora = use_adaln_lora
        self.adaln_lora_dim = adaln_lora_dim
        self.t_embedder = PairTimeEmbedder(
            model_channels, pair_timestep_feature_rank, use_adaln_lora=use_adaln_lora
        )
        ...
        te_module, _, _ = _load_transformer_engine()
        self.t_embedding_norm = te_module.pytorch.RMSNorm(
