...
from .._compat import DataType
from ..module.a2a_cp import MinimalA2AAttnOp
from ..networks.model_weights_stats import WeightTrainingStat
from ..networks.selective_activation_checkpoint import SACConfig as _SACConfig
from ..utils.context_parallel import split_inputs_cp
from .._compat import CosmosTextEncoderConfig
from .._compat import log
from .._compat import create_cuda_graph
...
def _load_transformer_engine():
    try:
        ...
        raise RuntimeError(
            "Transformer Engine is required during official model construction for normalization and fused rotary; attention backend selection is separate."
        ) from exc
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # Official Cosmos uses TE RMSNorm and fused rotary for every backend;
        # minimal_a2a selects only the attention dispatch below.
...
class VideoRopePosition3DEmb(VideoPositionEmb):
    ...
    def reset_parameters(self) -> None:
        ...
        self.seq = (
            torch.arange(max(self.max_h, self.max_w, self.max_t)).float().to(self.dim_spatial_range.device)
        )
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.use_adaln_lora:
            ...
            shift_mlp_B_T_D, scale_mlp_B_T_D, gate_mlp_B_T_D = self.adaln_modulation_mlp(emb_B_T_D).chunk(
                3, dim=-1
            )
...
class MiniTrainDIT(WeightTrainingStat):
    ...
    def __init__(
        ...
        self.t_embedding_norm = te_module.pytorch.RMSNorm(model_channels, eps=1e-6)
        self.init_weights()
